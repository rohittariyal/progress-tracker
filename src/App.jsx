...
function ProgressCircle({ value = 0, size = 120, stroke = 10, label = "", done = 0, total = 0 }) {
  ...
  return (
    <div className="relative flex flex-col items-center gap-2 select-none">
      ...
      <div className="text-xs text-zinc-600">{done}/{total} done</div>
    </div>
  );
}
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden bg-white text-zinc-900">
      ...
      <div className="mx-auto max-w-7xl p-6 md:p-10 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={spring} className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <motion.div layout className="rounded-2xl p-[1px] bg-gradient-to-br from-fuchsia-500/60 via-cyan-500/60 to-emerald-500/60 shadow-[0_0_40px_-14px_rgba(99,102,241,0.35)]">
            <div className="rounded-2xl bg-white/80 backdrop-blur px-5 py-3 flex items-center gap-5">
              <ProgressCircle value={data.overallProgress} label="Overall" done={overallCounts.done} total={totalItems} />
              <div>
                <div className="text-sm text-zinc-500">Overall progress</div>
                <div className="text-2xl font-bold">{data.overallProgress}%</div>
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <Card><div className="flex items-center justify-center">
            <ProgressCircle value={data.overallProgress} label="Overall" done={overallCounts.done} total={totalItems} />
          </div></Card>
          {data.phases.map((p)=> {
            ...
            return (
              <Card key={p.key}><div className="flex items-center justify-center">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-semibold flex items-center gap-2"><CalendarDays className="w-5 h-5 text-zinc-500"/> Today / Next Up</h2>
          ...
          <Card>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-semibold flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-emerald-600"/> Completed (All)</h2>
              <span className="text-xs text-zinc-500">Across phases</span>
            </div>
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {doneList.length ? doneList.map((d, i) => (
        ...
        <Card>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xl font-semibold flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-zinc-500"/> Recently Shipped</h2>
            <span className="text-xs text-zinc-500">Past 1–2 sprints</span>
          </div>
          <ul className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {data.recentlyShipped.map((s) => (
              <motion.li key={s} whileHover={{ x: 2 }} className="rounded-xl bg-white border border-zinc-200 shadow-sm px-3 py-2 text-sm">{s}</motion.li>
            ))}
          </ul>
        </Card>
          ...
          {data.phases.map((phase) => (
            <Card key={phase.key}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold flex items-center gap-2"><Icon name={phase.icon} className="w-4 h-4"/>{phase.title}</h3>
                <div className="h-2 w-40 rounded-full bg-zinc-200 overflow-hidden">
                  <div className={`h-full bg-gradient-to-r ${phase.color}`} style={{width: `${Math.round((phase.items.filter(i=>i.status!=="planned").length/phase.items.length)*100)}%`}} />
                </div>
              </div>
              <ul className="space-y-2">
                {phase.items.map((i) => (
        ...
        <Card hover={false}>
          <div className="flex items-center gap-2 text-zinc-600 text-sm">
            ...
            <span>Overall set at <span className="font-semibold text-zinc-800">{data.overallProgress}%</span>. Share this link with devs as the single source of truth.</span>
          </div>
